...
- **Phase 0 EXIT not fully signed off:** Google signup E2E needs the user's OAuth Client ID in BOTH `server/.env` (GOOGLE_CLIENT_ID) and `client/.env` (VITE_GOOGLE_CLIENT_ID), same client; plus a browser click-through of email + Google signup. mongosh seeds ✅, refresh rotation ✅ (backend + client-sim silent refresh), email flow ✅ (curl + client-sim). Do NOT start Phase 1 until the browser + Google check passes.
...
- 2026-07-08 · task 0.5 (client base) — DONE (code); browser + Google E2E pending
  ...
  - PENDING (Phase 0 EXIT): (1) browser click-through of email signup/login/logout via the AuthModal; (2) Google signup E2E — needs user to set GOOGLE_CLIENT_ID (server/.env) + VITE_GOOGLE_CLIENT_ID (client/.env) to the SAME OAuth client, then sign in with the Google button.
