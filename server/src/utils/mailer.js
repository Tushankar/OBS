...
export async function sendMail({ to, subject, html, text, type, userId, orderId, eventId, attachments }) {
  const log = await EmailLog.create({ type, toEmail: to, subject, status: 'QUEUED', userId, orderId, eventId });
  try {
    const info = await getTransport().sendMail({ from: env.EMAIL_FROM, to, subject, html, text, attachments });
    log.status = 'SENT';
